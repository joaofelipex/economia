...
<por-que-matplotlib>
matplotlib é a biblioteca padrão de gráficos do Python. Com ela você faz
desde um gráfico simples até figuras prontas para publicação em revista
acadêmica.
...
<personalização-completa-gráfico-publication-ready>
```python
from bcb import sgs
import matplotlib.pyplot as plt
...
Destaque em vermelho os que tiveram PIB negativo.

=== Exercício 3
<exercício-3>
Gere um scatter plot de inflação vs desemprego (Curva de Phillips) com
linha de tendência.

=== Exercício 4
<exercício-4>
Baixe o IPCA de 1995-2026 e faça um histograma + boxplot lado a lado
(use `subplots(1, 2)`).

=== Exercício 5
<exercício-5>
Recrie o gráfico "publication-ready" da seção 9, mas adicionando uma
terceira série (câmbio, código 1).
...
#figure(
  align(center)[#table(
    ...
    [Seaborn], [`sns.lineplot()`, `sns.regplot()`], [Gráficos mais
    bonitos],
  )]
  , kind: table
  )
